...
function [featureVector,coocMat,time] = cooc3d (varargin)
featureVector=NaN;
coocMat= NaN;
...
for iteration=1:size(data,4) %each new cube
    ...
    for dist =1:size(distance,1) %distance
        [harMat, coocMat(:,:,:,dist,iteration),time] = graycooc3d(...
...
function [harMat,coMat,time]= graycooc3d(I,distance,numLevels,numHarFeature,...
    offSet)
...
[harMat,time] = harFeatures(coMat,numHarFeature);
return
...
%feature)
function [harMat,time]= harFeatures(coMat, numHarFeature)
tic
%numHarFeature=12;
...
time(iteration,:)=[teng,tent,tcorr,tcont,tGen,tvar,tmea,tinset,tshade,tten,tmax,tinvar];
end
